...
namespace Lb6
{
    public partial class stirring_method : Form
    {
        public stirring_method()
        {
            InitializeComponent();
        }
        ...
        class Generator
        {
            ...
            {
                string step = Convert.ToString(Value);
                int step_len = step.Length;
                Value = Convert.ToInt32(step.Substring(step_len / 4) + step.Substring(0, step_len / 4)) +
                       Convert.ToInt32(step.Substring(step_len / 4 * 3) + step.Substring(0, step_len / 4 * 3));
                Value = step_len < Convert.ToString(Value).Length ? int.Parse(Convert.ToString(Value).Substring(1)) :  Value;
                return Value;
            }
        }
